...
import 'package:chess/logic/move_calculation/move_classes/direction.dart';
import 'package:chess/logic/move_calculation/move_classes/move_and_value.dart';
...
const KING_QUEEN_MOVES = [UP, UP_RIGHT, RIGHT, DOWN_RIGHT, DOWN, DOWN_LEFT, LEFT, UP_LEFT];
...
int _compareMoves(MoveAndValue a, MoveAndValue b, Player player, ChessBoard board) {
  return player == Player.player1 ? b.value.compareTo(a.value) : a.value.compareTo(b.value);
}
...
List<int> _pawnDiagonalAttacks(ChessPiece pawn, ChessBoard board) {
  List<int> moves = [];
  var diagonals = pawn.player == Player.player1 ? PAWN_DIAGONALS_1 : PAWN_DIAGONALS_2;
...
List<int> _rookMoves(ChessPiece rook, ChessBoard board, bool legal) {
  return _movesFromDirections(rook, board, ROOK_MOVES, true) + _rookCastleMove(rook, board, legal);
}
...
List<int> _kingMoves(ChessPiece king, ChessBoard board, bool legal) {
  return _movesFromDirections(king, board, KING_QUEEN_MOVES, false) + _kingCastleMoves(king, board, legal);
}
...
bool _canCastle(ChessPiece king, ChessPiece rook, ChessBoard board, bool legal) {
  if (rook.moveCount == 0 && king.moveCount == 0) {
    ...
    while (tile != king.tile) {
      tile += offset;
      if ((board.tiles[tile] != null && tile != king.tile) || (legal && _kingInCheckAtTile(tile, king.player, board))) {
        return false;
      }
    }
    return true;
...
List<int> _movesFromDirections(ChessPiece piece, ChessBoard board, List<Direction> directions, bool repeat) {
  List<int> moves = [];
