...
import 'package:chess/logic/chess_piece_sprite.dart';
import 'package:chess/logic/move_calculation/ai_move_calculation.dart';
import 'package:chess/logic/move_calculation/move_calculation.dart';
...
class ChessGame extends Game with TapDetector {
  ...
  void onTapDown(TapDownInfo info) {
    if (appModel.gameOver || !(appModel.isAIsTurn)) {
      ...
      if (touchedPiece == selectedPiece) {
        ...
        if (selectedPiece != null && touchedPiece != null && touchedPiece.player == selectedPiece.player) {
  ...
  void undoTwoMoves() {
    ...
    if (appModel.moveMetaList.length > 1) {
      _moveCompletion(appModel.moveMetaList[appModel.moveMetaList.length - 2], clearRedo: false, undoing: true, changeTurn: false);
    } else {
      _undoOpeningMove();
    }
  }
  ...
  void redoTwoMoves() {
    _moveCompletion(pushMSO(board.redoStack.removeLast(), board), clearRedo: false, updateMetaList: true);
    _moveCompletion(pushMSO(board.redoStack.removeLast(), board), clearRedo: false, updateMetaList: true);
  }
  ...
  void _drawBoard(Canvas canvas) {
    for (int tileNo = 0; tileNo < 64; tileNo++) {
      canvas.drawRect(
        ...
        Paint()..color = (tileNo + (tileNo / 8).floor()) % 2 == 0 ? appModel.theme.lightTile : appModel.theme.darkTile,
      );
    }
  }
