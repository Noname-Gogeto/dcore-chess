...
import 'package:chess/logic/move_calculation/move_classes/move_stack_object.dart';
import 'package:chess/logic/move_calculation/openings.dart';
...
class ChessBoard {
  ...
  void _addPiecesForPlayer(Player player) {
    ...
    for (var pieceType in KING_ROW_PIECES) {
      ...
      var pawn = ChessPiece(id + 1, ChessPieceType.pawn, player, kingRowOffset + pawnRowOffset + index);
...
MoveMeta push(Move move, ChessBoard board, {bool getMeta = false, ChessPieceType promotionType = ChessPieceType.promotion}) {
  var mso = MoveStackObject(move, board.tiles[move.from], board.tiles[move.to], board.enPassantPiece, List.from(board.possibleOpenings));
...
void _filterPossibleOpenings(ChessBoard board, Move move) {
  board.possibleOpenings =
      board.possibleOpenings.where((opening) => opening[board.moveCount] == move && opening.length > board.moveCount + 1).toList();
}
...
bool _promotion(ChessPiece movedPiece) {
  return movedPiece.type == ChessPieceType.pawn && (tileToRow(movedPiece.tile) == 7 || tileToRow(movedPiece.tile) == 0);
}

bool _canTakeEnPassant(ChessPiece movedPiece) {
  return movedPiece.moveCount == 1 && movedPiece.type == ChessPieceType.pawn && (tileToRow(movedPiece.tile) == 3 || tileToRow(movedPiece.tile) == 4);
}

bool _inEndGame(ChessBoard board) {
  return (_queensForPlayer(Player.player1, board).isEmpty && _queensForPlayer(Player.player2, board).isEmpty) ||
