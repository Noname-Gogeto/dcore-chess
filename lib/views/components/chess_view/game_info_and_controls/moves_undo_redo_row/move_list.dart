...
import 'package:chess/views/components/shared/text_variable.dart';
...
class MoveList extends StatelessWidget {
  ...
  String _moveToString(MoveMeta meta) {
    ...
    if (meta.kingCastle) {
      ...
      String ambiguity = meta.rowIsAmbiguous ? _colToChar(tileToCol(meta.move.from)) : '';
      ambiguity += meta.colIsAmbiguous ? '${8 - tileToRow(meta.move.from)}' : '';
      String takeString = meta.took ? 'x' : '';
      String promotion = meta.promotion ? '=${_pieceToChar(meta.promotionType)}' : '';
      String row = '${8 - tileToRow(meta.move.to)}';
      String col = _colToChar(tileToCol(meta.move.to));
      move = '${_pieceToChar(meta.type)}$ambiguity$takeString' '$col$row$promotion';
    }
